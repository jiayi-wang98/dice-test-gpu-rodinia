...
#define STR_SIZE 256
#define DEVICE 0
#define HALO 1 // halo width along one
...
#define M_SEED 9
int pyramid_height;
static int max_iterations = INT_MAX; // Allow all iterations
                ...
                int border)
{
    ...
    for (int i=0; i<iteration ; i++){ 
        ...
        if( IN_RANGE(tx, i+1, BLOCK_SIZE-i-2) && isValid){
            ...
            // Debug output for index 3
            if (xidx == 3) {
                printf("GPU iter %d, idx %d: left=%d, up=%d, right=%d, shortest=%d, wall[%d]=%d, result=%d\n",
                       i, xidx, left, up, right, shortest, index, gpuWall[index], result[tx]);
            }
        }
        __syncthreads();
        if(i==iteration-1)
        ...
        for (int bx = 0; bx < (cols + small_block_cols - 1) / small_block_cols; bx++) {
            ...
            for (int tx = 0; tx < BLOCK_SIZE; tx++) {
                ...
                if (IN_RANGE(tx, i+1, BLOCK_SIZE-i-2) && isValid && IN_RANGE(xidx, 0, cols-1)) {
                    int left = src[W + blkX];
                    int up = src[xidx];
                    int right = src[E + blkX];
                    int shortest = MIN(left, up);
                    shortest = MIN(shortest, right);
                    int index = cols * (startStep + i + 1) + xidx;
                    dst[xidx] = shortest + wall[index];
                    // Debug output for index 3
                    if (xidx == 3) {
                        printf("CPU iter %d, idx %d: left=%d, up=%d, right=%d, shortest=%d, wall[%d]=%d, result=%d\n",
                               i, xidx, left, up, right, shortest, index, wall[index], dst[xidx]);
                    }
                }
            }
        }
	 ...
	 int pyramid_height, int blockCols, int borderCols)
{
	...
	for (int t = 0; t < rows-1; t+=pyramid_height) {
...
#endif
        calc_path_cpu_partial(data, cpuSrc, cpuDst, cols, rows, t, iterations, borderCols);
#ifdef TIMING
        ...
        for (int i = 0; i < cols; i++) {
            if (result[i] != cpuDst[i]) {
                match = false;
                printf("Mismatch at iteration %d, index %d: GPU = %d, CPU = %d\n", 
                       cuda_kernel_called_times, i, result[i], cpuDst[i]);
                break;
            }
        }
        ...
        // Swap CPU buffers
        int tempCpu = cpuSrcIdx;
        cpuSrcIdx = cpuDstIdx;
        cpuDstIdx = tempCpu;

        // Copy CPU results to GPU source for next iteration
        cudaMemcpy(gpuResult[dst], cpuDst, sizeof(int)*cols, cudaMemcpyHostToDevice);
