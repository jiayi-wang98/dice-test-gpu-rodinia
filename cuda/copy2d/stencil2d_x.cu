// 5-point with cross-block fallback for ALL directions (mimicking stencil2d.cu).
#include <stdio.h>
#include <stdlib.h>
__global__ void k(const int *in, int *out, int W, int H) {
    ...
    int down  = (ty < 15) ? tile[ty+1][tx] : ((gy < H-1) ? in[(gy+1)*W + gx] : center);
    int left  = (tx > 0)  ? tile[ty][tx-1] : ((gx > 0)   ? in[gy*W + (gx-1)] : center);
    int right = (tx < 15) ? tile[ty][tx+1] : ((gx < W-1) ? in[gy*W + (gx+1)] : center);
...
int main() {
    ...
    printf("stencil2d_x: %s (%d/%d wrong)\n", bad==0?"OK":"FAIL", bad, W*H);
    return bad?1:0;
}
