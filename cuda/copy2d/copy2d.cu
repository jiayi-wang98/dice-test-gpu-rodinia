#include <stdio.h>
#include <stdlib.h>
__global__ void copy2d_kernel(const int *in, int *out, int W, int H) {
    int gx = blockIdx.x * blockDim.x + threadIdx.x;
    int gy = blockIdx.y * blockDim.y + threadIdx.y;
    if (gx < W && gy < H) out[gy*W + gx] = in[gy*W + gx] + 1;
}
int main(int argc, char **argv) {
    ...
    copy2d_kernel<<<g, b>>>(d_in, d_out, W, H);
    cudaDeviceSynchronize();
    cudaMemcpy(h_out, d_out, bytes, cudaMemcpyDeviceToHost);
    int bad = 0;
    for (int i = 0; i < W*H; i++) {
        if (h_out[i] != h_in[i] + 1) {
            if (bad < 5) printf("Mismatch at %d: GPU=%d expected=%d\n", i, h_out[i], h_in[i]+1);
            bad++;
        }
    }
    printf("copy2d: %s (%dx%d, %d/%d wrong)\n", bad==0?"OK":"FAIL", W, H, bad, W*H);
    return bad ? 1 : 0;
}
